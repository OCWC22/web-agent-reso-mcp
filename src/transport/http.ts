...
async function handleSSERequest(
  ...
  const serverInstance = createStandaloneServer(config.apiKey, config.agentMailBaseUrl);
...
async function createNewSession(
  ...
  const transport = new StreamableHTTPServerTransport({
    ...
    onsessioninitialized: (sessionId) => {
      ...
      console.log('New web-agent-reso-mcp session created:', sessionId);
    },
  });
  ...
  transport.onclose = () => {
    if (transport.sessionId) {
      ...
      console.log('web-agent-reso-mcp session closed:', transport.sessionId);
    }
  };
...
function logServerStart(config: Config): void {
  ...
  console.log(`[web-agent-reso-mcp] MCP Server listening on ${displayUrl}`);
