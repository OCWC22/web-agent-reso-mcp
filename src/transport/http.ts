...
export function startHttpTransport(config: Config & { port: number }): void {
...
async function handleSSERequest(_req: IncomingMessage, res: ServerResponse, config: Config): Promise<void> {
  const serverInstance = createStandaloneServer(config.openaiApiKey);
...
async function createNewSession(req: IncomingMessage, res: ServerResponse, config: Config): Promise<void> {
  ...
  const transport = new StreamableHTTPServerTransport({
    ...
    onsessioninitialized: (sessionId) => {
      ...
      console.log('New browser-use session created:', sessionId);
    },
  });
  ...
  transport.onclose = () => {
    if (transport.sessionId) {
      ...
      console.log('browser-use session closed:', transport.sessionId);
    }
  };
...
function handleHealthCheck(res: ServerResponse): void {
  ...
  res.end(
    JSON.stringify({ status: 'healthy', timestamp: new Date().toISOString(), service: 'browser-use-mcp', version: '0.2.0' })
  );
}
...
function logServerStart(config: Config & { port: number }): void {
  ...
  console.log(`[browser-use] MCP Server listening on ${displayUrl}`);
